...
const state = { ws: null, hosts: [], host: '', session: '', retry: null, ready: false };
...
function render() {
  ...
  const oldHost = state.host, oldSession = state.session;
  ...
  for (const s of state.hosts.find(h => h.id === state.host)?.sessions || []) {
    option(session, s.id, `${s.id} (${s.harness})`); option(target, s.id, `${s.id} (${s.harness})`);
  }
  state.session = [...session.options].some(o => o.value === oldSession) ? oldSession : session.options[0]?.value || '';
  session.value = state.session;
  if (oldHost !== state.host || oldSession !== state.session) { term.clear(); if (state.session) emit({ type: 'replay', ...selected() }); }
  fit.fit(); resize();
}
...
$('session').onchange = () => { state.session = $('session').value; term.clear(); emit({ type: 'replay', ...selected() }); resize(); term.focus(); };
...
$('send').onclick = () => { const text = $('message').value.trim(); if (!text || !state.session || !$('target').value) return;
  emit({ type: 'send', ...selected(), to: $('target').value, text }); $('message').value = ''; };
